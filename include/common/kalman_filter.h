...
private:
    ...
    // Added member variables to replace locals in methods
    double K_pos, K_vel, K_acc; // Kalman gain components
    double S; // Innovation covariance
    double innovation; // Measurement innovation
...
public:
    inline KalmanFilter(
        ...
        : position(init_pos), velocity(init_vel), acceleration(init_acc),
          ...
          K_pos(0), K_vel(0), K_acc(0), S(0), innovation(0) {
    }
    ...
    inline void predict(double dt) {
        ...
        // Temporary variables to store old values - not static as they need
        // to be unique per filter instance
        double old_P_pos = P_pos;
        double old_P_vel = P_vel;
        double old_P_acc = P_acc;
        double old_P_pos_vel = P_pos_vel;
        double old_P_vel_acc = P_vel_acc;
        double old_P_pos_acc = P_pos_acc;
    ...
    inline void update(double& measurement) {
        // Kalman gain calculation for position measurement
        S = P_pos + R; // Innovation covariance
        K_pos = P_pos / S;
        K_vel = P_pos_vel / S;
        K_acc = P_pos_acc / S;
    ...
    // Add getter for measurement noise
    inline double getR() const { return R; }
    
    // Add setter for measurement noise
    inline void setR(double newR) { R = newR; }

    // Add getters for all state variables
    inline double get_P_pos() const { return P_pos; }
    inline double get_P_vel() const { return P_vel; }
    inline double get_P_acc() const { return P_acc; }
};