...
void moveFwd_2(float dist, int timeout, float atSpeed, bool async=true) {
    lemlib::Pose start = chassis.getPose();
    if(async) {
        pros::Task([&]{moveFwd_2(dist, timeout, atSpeed, false);});
    } else {
        uint32_t start_time = pros::millis();
        float thetaRad = convert::radToDeg(chassis.getPose().theta);
        float deltaX = dist * std::cos(thetaRad);
        float deltaY = dist * std::sin(thetaRad);
        chassis.tank(atSpeed, atSpeed);
        while (start_time+timeout > pros::millis()) {
            if (std::abs(chassis.getPose().x - start.x) >= deltaX && std::abs(chassis.getPose().y - start.y) >= deltaY) {
                break;
            }
            delay(10);
        }
...
void redAuton() {
    ...
    chassis.follow(comp_white_1_n_txt, 15, 2500, FORWARD, SYNC);
    chassis.tank(0, 0, 0);
    delay(100);
    mogoRushClamp.retract();
    chassis.swingToHeading(163, lemlib::DriveSide::RIGHT, 1500, {.minSpeed = (int)ceil(127/2.5)}, SYNC);
    // chassis.moveToPose(chassis.getPose().x+4, chassis.getPose().y-11, 170, 1500, {.maxSpeed = 127/2}, SYNC);
    moveFwd_2(10, 1500, 90, SYNC);
    // chassis.turnToHeading(0, 1200, {.direction=lemlib::AngularDirection::CW_CLOCKWISE, .minSpeed = 127/3}, ASYNC);
